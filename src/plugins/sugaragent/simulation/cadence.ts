export type CadenceTier = 'near' | 'mid' | 'far';

export interface CrowdTownCadenceOptions {
  maxNpcUpdatesPerTick?: number;
  nearCadenceTicks?: number;
  midCadenceTicks?: number;
  farCadenceTicks?: number;
  nearRadius?: number;
  midRadius?: number;
  activeBeatNpcId?: string;
  seed?: number;
}

export interface CrowdTownCadenceReport {
  ...
  planner: {
    ticksWithFarPlanning: number;
    farPlanningUpdates: number;
  };
  transitions: number;
  continuity: {
    average: number;
  ...
  activeBeat: {
    nearTicks: number;
    updatedWhileNear: number;
    responsiveness: number;
    farAutoCompletions: number;
  };
}

interface CrowdTownNpcSeed {
  id: string;
  baseDistance: number;
...
interface CrowdTownNpcState {
  id: string;
  baseDistance: number;
  driftAmplitude: number;
  cadenceDivisor: number;
  phase: number;
  hasActiveBeat: boolean;
  tier: CadenceTier;
  lastUpdatedTick: number;
  continuity: number;
}

const NPC_SEEDS: CrowdTownNpcSeed[] = [
...
function tierForDistance(distance: number, nearRadius: number, midRadius: number): CadenceTier {
  if (distance <= nearRadius) return 'near';
  if (distance <= midRadius) return 'mid';
  return 'far';
}

function cadenceForTier(tier: CadenceTier, config: CrowdTownCadenceReport['config']): number {
  if (tier === 'near') return config.nearCadenceTicks;
  if (tier === 'mid') return config.midCadenceTicks;
  return config.farCadenceTicks;
}

function continuityDecayForTier(tier: CadenceTier): number {
  if (tier === 'near') return 0.0015;
  if (tier === 'mid') return 0.003;
  return 0.005;
}

function continuityBoostForTier(tier: CadenceTier): number {
  if (tier === 'near') return 0.05;
  if (tier === 'mid') return 0.035;
  return 0.025;
}

function distanceAtTick(npc: CrowdTownNpcState, tick: number): number {
  const wave = Math.abs(Math.sin((tick + npc.phase) / npc.cadenceDivisor));
  return npc.baseDistance + wave * npc.driftAmplitude;
}

function createNpcStates(activeBeatNpcId: string): CrowdTownNpcState[] {
...
function updatePriority(npc: CrowdTownNpcState, staleness: number): number {
  if (npc.hasActiveBeat && npc.tier === 'near') return 0 - staleness * 0.01;
  if (npc.tier === 'near') return 1 - staleness * 0.01;
  if (npc.hasActiveBeat && npc.tier === 'mid') return 2 - staleness * 0.01;
  if (npc.tier === 'mid') return 3 - staleness * 0.01;
  if (npc.hasActiveBeat) return 4 - staleness * 0.01;
...
export function runCrowdTownCadenceSimulation(
  ticks: number,
  options: CrowdTownCadenceOptions = {},
): CrowdTownCadenceReport {
  const totalTicks = toPositiveInt(ticks, 1);
  const config: CrowdTownCadenceReport['config'] = {
  ...
  for (let tick = 1; tick <= totalTicks; tick += 1) {
    const due: Array<{ npc: CrowdTownNpcState; staleness: number; }> = [];
    let sawFarDue = false;
