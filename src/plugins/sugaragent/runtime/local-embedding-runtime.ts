...
const EMBEDDING_MODEL_DIR = path.resolve('src/plugins/sugaragent/runtime/bundle/embeddings/all-MiniLM-L6-v2');
const EMBEDDING_MODEL_PATH = path.join(EMBEDDING_MODEL_DIR, 'onnx', 'model_quantized.onnx');
const EMBEDDING_VOCAB_PATH = path.join(EMBEDDING_MODEL_DIR, 'vocab.txt');
const EMBEDDING_CONFIG_PATH = path.join(EMBEDDING_MODEL_DIR, 'config.json');
const EMBEDDING_TOKENIZER_PATH = path.join(EMBEDDING_MODEL_DIR, 'tokenizer.json');
...
async function loadTokenizerAssets(): Promise<TokenizerAssets> {
  const [vocabRaw, configRaw, tokenizerRaw] = await Promise.all([
    fs.readFile(EMBEDDING_VOCAB_PATH, 'utf8'),
    fs.readFile(EMBEDDING_CONFIG_PATH, 'utf8'),
    fs.readFile(EMBEDDING_TOKENIZER_PATH, 'utf8'),
  ]);
...
async function getEmbeddingSession(): Promise<ort.InferenceSession> {
  sessionPromise ??= ort.InferenceSession.create(EMBEDDING_MODEL_PATH, {
...
export async function getLocalEmbeddingRuntimeHealth(): Promise<LocalEmbeddingRuntimeHealth> {
  try {
    const [assets] = await Promise.all([
      getTokenizerAssets(),
      fs.access(EMBEDDING_MODEL_PATH),
    ]);
    return {
      ok: true,
