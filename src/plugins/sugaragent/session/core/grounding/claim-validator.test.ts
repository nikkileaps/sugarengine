...
describe('SugarAgent claim validator', () => {
  ...
  it('treats acknowledgement interjections as non-factual', () => {
    const claims = extractClaimUnits('Yes! The Wordlark Hollow Resort is just outside town.');

    expect(claims).toHaveLength(2);
    expect(claims[0]?.factual).toBe(false);
    expect(claims[0]?.nonFactualReason).toBe('acknowledgement');
    expect(claims[1]?.factual).toBe(true);
  });
  ...
  it('does not require repair for acknowledgement-only utterances', () => {
    const grounding = validateGroundedClaims({
      utterance: 'Yes!',
      queryType: 'world_query',
      evidenceEntries: [],
