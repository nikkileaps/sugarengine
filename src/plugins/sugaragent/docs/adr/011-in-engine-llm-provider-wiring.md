# ADR-011: SugarAgent In-Engine LLM Provider Wiring
...
### Phase 11A: Runtime Bridge for In-Engine Calls
...
### Phase 11B: Replace Deterministic Primary Turn Path
...
### Phase 11C: Authoring Context Injection
...
### Phase 11D: Runtime UX + Observability
...
1. `runAgentTurn(...)` now uses `LocalLLMProvider` as the primary in-engine generation path.
2. Authoring context injection is wired (`persona`, `tone`, `constraints`, `loreScopes`, identity fields, and `globalSafetyBounds`).
3. Runtime health and outcome diagnostics are surfaced through plugin runtime state and preview debug paths.
4. Runtime bridge set includes:
   - `MockLocalRuntimeBridge` (tests/dev),
   - `HttpLocalRuntimeBridge` (preview/browser),
   - `TauriLocalRuntimeBridge` (native app bridge client with optional HTTP fallback for dev parity),
   - native Tauri command backend `sugaragent_runtime_bridge` in `src-tauri/src/lib.rs` supporting:
     - `health`
     - `loadModel`
     - `generateStructured`
     - `embed`
     - `unloadModel`
5. Tauri bundle resources now include SugarAgent local runtime/model assets for packaged desktop builds (`src-tauri/tauri.conf.json`).

Follow-on hardening (separate from ADR-011 completion):

1. Improve command-level parity with session runtime internals (full retrieval/governance parity and richer diagnostics surface).
2. Extend native packaging strategy for mobile targets and runtime-size constraints.
