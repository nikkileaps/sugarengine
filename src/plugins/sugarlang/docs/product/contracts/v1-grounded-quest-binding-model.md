...
- how a taught referent maps to an in-world object
...
The product should not replace that with a disconnected prompt unless the scene truly has no grounded quest action available.

## Binding Chain

For V1, a grounded teaching referent should be able to map across this chain:

1. semantic referent
   - what concept is being taught
2. world referent
   - which object, region, or landmark in the scene carries that meaning
...
Not every scene needs every step, but the product should bind as many of them as the authored quest supports.

## Product Rules

### 1. One Taught Referent, One Real Quest Identity

If `maleta roja` is the taught referent in the scene, the product should know:
...
- teaching units that should stay attached to the binding
...
| Band variant | World object | Visible attributes | Attached teaching units |
| --- | --- | --- | --- |
...
Some scenes will not include pickup or inventory.
...
The rule is not "every scene must have inventory."

The rule is "bind to the real quest loop as far as the scene allows."

## AI Generation Contract

When the AI assistant generates or updates a scene, it should try to infer:
...
- which vocabulary should remain attached to that referent
...
For every grounded scene, product review should be able to answer:
...
If the answer is no, the scene is not grounded strongly enough.
