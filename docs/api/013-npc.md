...
| `interactionMode` | `'scripted' \| 'agent' \| 'hybrid'` | Optional SugarAgent routing mode. Default `scripted`. |

Editor note: this field is configurable in NPC Detail under the `SugarAgent` section.
...
2. **Behavior tree** - evaluate the NPC's BT (skipped when `interactionMode: "agent"`); if it returns a `dialogue` action, use that
3. **Plugin resolution** - only when SugarAgent is enabled and NPC mode is `agent` or `hybrid`
4. **Default dialogue** - fall back to the NPC's `defaultDialogue`

When an active SugarAgent beat contract is bound to that NPC, plugin turns can be
budget-limited (`maxTurns`) and route to scripted fallback (`fallbackScriptId`).
