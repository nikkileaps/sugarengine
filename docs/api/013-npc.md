...
**Condition nodes** (yellow) - check game state, return success or failure:
...
Built-in conditions (`questStage`, `hasItem`, `hasFlag`) and actions (`dialogue`, `moveTo`, `wait`, `setFlag`, `emitEvent`) are handled automatically. For custom types:
